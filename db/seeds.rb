...
3.times do
  i ||=0
  i += 1
  group = Group.create(name: "House " + Faker::GameOfThrones.house, admin_id: i)
  group.users << User.find(i)
end

10.times do
  user = User.create(name: Faker::Twitter.screen_name)
  3.times do
...
GroupMovie.all.each do |group_movie|
  5.times do
    ...
    parent_comment = Comment.new(user: user, group_movie: group_movie, content: Faker::BackToTheFuture.quote, vote: 1, parent_id: nil)
    ...
    3.times do
      user = group_movie.group.users.all.sample
      child_comment = Comment.create(user: user, group_movie: group_movie, content: Faker::BackToTheFuture.quote, vote: 1, parent_id: parent_comment.id)
      user = group_movie.group.users.all.sample
      grandchild_comment = Comment.create(user: user, group_movie: group_movie, content: Faker::BackToTheFuture.quote, vote: 1, parent_id: child_comment.id)
    end
  end
end
